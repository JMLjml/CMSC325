...
public class StrategyProbabilistic extends Strategy {
  ...
  // OPCODE + WEIGHT + REPEAT + INSTRUCTION_TYPE + INSTRUCTION_SUBTYPE +
  // NUM_STEPS_BACK
...

  {
    ...
    // We need to save the weighted probabilities and the move that should be
    // made for each instruct. type.
    ...
    } // Our current move will be random
...

  {
...

    {
...

      {
        ...
        // will play defect, we can attempt to break the tit for tat by playing
        // a random
...

  {
...

    {
      ...
      if (relativeType == COPY
          && playerMoves.get(i - numStepsBack) == opponentMoves.get(i))
...

  {
    ...
    String weightString = instruction.substring(INDEX_WEIGHT_BEGIN,
        INDEX_WEIGHT_END + 1);
    ...
    // 2 ^ 4 = 16 which can represent from 0 - 15. We want are probability to
    // represent from 0 to 1
